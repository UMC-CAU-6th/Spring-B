...
public class ReviewRestController {
    ...
    @Operation(summary = "자신의 리뷰 목록 조회 API",description = "자신이 작성한 리뷰들의 목록을 조회하는 API이며, 페이징을 포함합니다. query String 으로 page 번호를 주세요")
    ...
    @Parameters({
            @Parameter(name = "nickname", description = "유저의 닉네임, path variable 입니다!"),
            @Parameter(name = "page", description = "페이지 번호, 0번이 1 페이지 입니다."),
    })
    public ApiResponse<StoreResponseDTO.ReviewPreViewListDTO> getReviewList(@PathVariable(name = "nickname") String nickname, @RequestParam(name = "page") Integer page){
        return ApiResponse.onSuccess(StoreConverter.reviewPreViewListDTO(storeQueryService.getReviewList(nickname, page)));
    }
}
